...
class TripletLoss(nn.Module):
    ...
    def forward(self, emb, label, clot_feats_s):
        ...
        mat_dist_clot_feats_s = cosine_dist(clot_feats_s, clot_feats_s)
        assert mat_dist.size(0) == mat_dist.size(1)
        N = mat_dist.size(0)
        mat_sim = label.expand(N, N).eq(label.expand(N, N).t()).float()
        for n_index in range(N):
            # import pdb;pdb.set_trace()
            mat_dist_clot_feats_s[n_index] = mat_dist_clot_feats_s[n_index] * mat_sim[n_index]
        # import pdb;pdb.set_trace()
        mat_dist = mat_dist - mat_dist_clot_feats_s
        # import pdb;pdb.set_trace()
