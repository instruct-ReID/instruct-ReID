...
# from reid.loss import TripletLoss, LabelSmoothing, CenterLoss, CircleLoss
...
class TransreidTwobranchTrainer(BaseTrainer):
    ...
    def run(self, inputs):
        ...
        if self.args.transe_loss:
            ...
            # loss_ce += self.ce_loss(logits2_pos, targets)
            # loss_tr += self.triplet_loss(feat_pos, targets)[0]
            # loss_ce_bme += self.ce_loss(logits1_pos, targets)
